...
<body data-spy="scroll" data-offset="0" data-target="#theMenu" style="background:#2196F3">

<link href="//maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css" rel="stylesheet">

<!--code for searching alumni citywise,countrywise and statewise-->
<center><h2 style="	font-family:'Arima Madurai', cursive;">Read Alumnies Blogs here....</h2></center>
<form method="post" action="BlogPage.php">
<div class="row">
...
<label>Search Statewise</label><br>
...
<option value="West Bengal">West Bengal</option>
  ...
  <div class="col-sm-4">
<center>
<label>Search citywise</label><br>
...
<option>Other</option>
  ...
  <div class="col-sm-4">
 <center>
  <button type="submit" name="search" class="btn btn-default btn-sm" style="margin-top:20px">
...
if(isset($_POST['search']))
{
	...
	$state=$_POST['state'];
	$city=$_POST['city'];
	$qur2="select * from registration";
	$res2=mysql_query($qur2,$connection);
	$username='';
$Button_ID=1;	
while($row=mysql_fetch_array($res2))
{	
if(strcmp($city,'')!=0 && strcmp($state,'')==0 && strcmp($country,'')==0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and city='".$city."'";
    goto execute;
}
if(strcmp($state,'')!=0 && strcmp($city,'')==0 && strcmp($country,'')==0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and state='".$state."'";
	goto execute;
}
if(strcmp($state,'')==0 && strcmp($city,'')==0 && strcmp($country,'')!=0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and country='".$country."'";
	goto execute;
}
if(strcmp($state,'')!=0 && strcmp($city,'')==0 && strcmp($country,'')!=0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and country='".$country."' and state='".$state."'";
	goto execute;
}
if(strcmp($state,'')==0 && strcmp($city,'')!=0 && strcmp($country,'')!=0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and country='".$country."' and city='".$city."'";
	goto execute;
}
if(strcmp($state,'')!=0 && strcmp($city,'')!=0 && strcmp($country,'')==0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and state='".$state."' and city='".$city."'";
	goto execute;
}
if(strcmp($state,'')!=0 && strcmp($city,'')!=0 && strcmp($country,'')!=0)
{
	$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."' and country='".$country."' and city='".$city."' and state='".$state."'";
	goto execute;
}	
if(strcmp($state,'')==0 && strcmp($city,'')==0 && strcmp($country,'')==0)
{	
$qur="select name,postincompany,nameoforganization,username,city from registration where BINARY username='".$row['username']."'";
	...
	$qur1="select * from profiles where BINARY username='".$var."'";
	...
	$row1=mysql_fetch_array($res1);
      ...
      $qur_button="update registration SET Button_ID='".$Button_ID."' where BINARY username='".$var."'";
	  
      $res_button=mysql_query($qur_button,$connection);
	  
	
	//echo "Buttton:".$Buuton_ID."PAth:".$row1['path']."";
	if(strcmp($row1['path'],'')!=0)
	{
...
?>
		   ...
		   <div class="searchbox">
            ...
            <div class="card hovercard">
                <form method="post" action="Blog/blog1.php">
				<div class="cardheader">

                </div>
                <div class="avatar">
                    <img alt="" src="<?php echo $row1['path'] ?>">
                </div>
                <div class="info">
                    <div class="title">
                    <?php echo $name['name']?>
					</div>
                    <div class="desc"><?php echo $name['postincompany']?></div>
                    <div class="desc"><?php echo $name['nameoforganization']?></div>
                    <div class="desc"><?php echo $name['city']?></div>
               
			   </div>
				<input type="hidden" id="username" name="username" value="<?php echo $var;?>"/> 
				 <input type="hidden" id="var"  name="var" value="<?php echo $Button_ID ?>"	/>
				 <button  type="submit" name="submit" class="btn btn-info btn-lg" id="pro_button" ><span class="glyphicon glyphicon-user"></span> Read Blog</button>
                 <br><br>
				 </form>
            </div>
...
if($Button_ID==1)
	echo "<p><span class='glyphicon glyphicon-search'></span>&nbsp&nbsp&nbspNo Search Found...</p>";

}
else
{
$Button_ID=1;	
$qur1="select * from profiles";
$res1=mysql_query($qur1,$connection);
while($row=mysql_fetch_array($res1))
{	
$qur="select name,postincompany,nameoforganization,city from registration where BINARY username='".$row['username']."'";
  $res=mysql_query($qur,$connection);  
  $name=mysql_fetch_array($res);
      $var=$row['username'];
	

      $qur_button="update Blogs SET Button_ID='".$Button_ID."' where BINARY username='".$var."'";
...
?>
		   ...
		   <div class="searchbox">
            ...
            <div class="card hovercard">
				...
				<input type="hidden" id="username" name="username" value="<?php echo $var;?>"/> 
				 <input type="hidden" id="var"  name="var" value="<?php echo $Button_ID ?>"	/>
				 <button  type="submit" name="submit" class="btn btn-info btn-lg" id="pro_button" ><span class="glyphicon glyphicon-eye-open"></span>  Read Blog</button>
                 <br><br>
				 </form>
            </div>
...
if($Button_ID==1)
		echo "<p><span class='glyphicon glyphicon-search'></span>&nbsp&nbsp&nbspNo Record Exist...</p>";
